...
1. **JD file** — `companies/[Company]/roles/[Position]-[ID].md`
...
**Local sqlite update sequence:**

```python
import sqlite3

# Open the sqlite database directly from the workspace
conn = sqlite3.connect('/path/to/workspace/comparison-matrix.sqlite')
cur = conn.cursor()
...
After each interview stage (see also `{references_folder}/interview-debrief.md`):
