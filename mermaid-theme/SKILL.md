...
- Use **class-based styling** when the same component appears in multiple diagrams (e.g., `litellm` on discovery, ds9, enterprise)
- Always include `color:#fff` for colored backgrounds — ensures text readability
- Don't mix inline and class-based styling for the same node — pick one pattern per diagram
